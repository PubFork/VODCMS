define(['jquery', 'bootstrap', 'backend', 'addtabs', 'adminlte', 'form'], function ($, undefined, Backend, undefined, AdminLTE, Form) {
    var Controller = {
        index: function () {
			...
			$(document).on("click", "[data-url].btn-detail", function(e) {
					var url = $(this).data("url");
					var name = $(this).data("name");
					Backend.api.open(url, name);
			});

            //读取FastAdmin的更新信息
            /*$.ajax({
                url: Config.site.apiurl + 'news/index/',
                type: 'post',
                dataType: 'jsonp',
            ...
            var checkupdate = function (ignoreversion, tips) {
                $.ajax({
				    url: Config.site.apiurl + 'version/check/',
                    type: 'post',
            ...
            $(document).on('click', ".logout", function() {
            	...
            	layer.confirm('确认退出吗?', {
            		...
            		$.get(target).success(function(data) {
            			if (data.code == 1) {
            				Toastr.success(data.msg ? data.msg : '退出成功');
            				if (data.url) {
								setTimeout(function() {
            				...
            				Toastr.error(data.msg ? data.msg : '退出失败');
            			}
            		});
            	});
            });	
          ...
          //绑定tabs事件
            ...
            function change_skin(cls) {
                if (!$("body").hasClass(cls)) {
                    ...
                    $("body").addClass(cls);
                    store('skin', cls);
					var cssfile = Backend.api.cdnurl("admin/zanpiancms/css/skins/" + cls + ".css");
